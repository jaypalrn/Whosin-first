...
import { boldFont, semiBoldFont } from '../../utilities/styles/Themes'
    ...
    return (
        <View style={[style, { backgroundColor: backgroundColor ? backgroundColor : 'transparent', width: windowWidth }]}>
            ...
            <View style={{ height: Platform.OS == 'ios' ? 44 : 55, justifyContent: 'center', marginBottom: 3 }}>
                ...
                {isBackButton &&
                    <TouchableOpacity style={{ position: 'absolute', start: 20, paddingVertical: 5, paddingHorizontal: 10, borderRadius: 20, }} onPress={leftIconOnPress}>
                        <BackArrowIcon
                ...
                {withBackButton &&
                    <TouchableOpacity style={{ position: 'absolute', justifyContent: 'center', height: '100%' }} onPress={leftBtnOnPress}>
                        <Text style={[boldFont({ color: withBackColor ? withBackColor : Colors.white.color, size: withBackSize ? withBackSize : 14 }), { alignSelf: titlePosition ? titlePosition : 'flex-start', start: 65, position: 'absolute', }]}>{withBackButton}</Text>
                    </TouchableOpacity>
                }
